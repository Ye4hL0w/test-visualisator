...
class VisGraph extends HTMLElement {
  ...
  createForceGraph() {
    ...
    const simulation$1 = simulation(this.nodes)
      .force('link', link$2(this.links).id(d => d.id).distance(linkDistance))
      .force('charge', manyBody().strength(-200))
      .force('center', center(width / 2, height / 2))
      .force('collision', collide().radius(d => getNodeRadius(d) + 5)) // Padding
      .force('x', x$1(width / 2).strength(0.1))
      .force('y', y$1(height / 2).strength(0.1));
